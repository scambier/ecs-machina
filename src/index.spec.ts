...
import { Component, ComponentFactory, Entity, mergeDeep, World } from './index'
...
describe('The World', () => {
  ...
  beforeEach(() => {
    world = new World()
    Position = Component('Position', { x: 5, y: 6 })
    Velocity = Component('Velocity', { dx: 0, dy: 0 })
    IsFlag = Component('IsFlag', {})
    Tag = Component('Tag', {})
  })
  ...
  describe('destroy', () => {
    it('removes an entity from the world', () => {
      ...
      expect(world.getComponent(entityA, Tag)).toBeUndefined()
    })
  })
  ...
  describe('query', () => {
    ...
    it('fetches components after the entity has been modified', () => {
      ...
      // Set components
      world.setComponents(entity, velocity)
      world.setComponents(entity, Tag())
...
describe('Components', () => {
  ...
  it('returns their own key when displayed as strings', () => {
    const OtherComponent = Component('Other', {})
    expect(Position._cmpId).not.toEqual(Velocity._cmpId)
    expect(Velocity._cmpId).not.toEqual(OtherComponent._cmpId)
  })
